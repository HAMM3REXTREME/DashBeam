...
Rectangle {
    ...
    property real barEndThickness: 0.0025
    ...
    Rectangle {
        ...
        width: horizontal ? ((rpmBar.maxValue - rpmBar.redlineValue)
                             / rpmBar.maxValue) * parent.width : parent.width * 0.25
        // Inverse of statement above.
        height: horizontal ? parent.height * 0.25 : ((rpmBar.maxValue - rpmBar.redlineValue)
                                                     / rpmBar.maxValue) * parent.height
        // horizontal bar, labels on top --> start from parent.bottom
        anchors.bottom: horizontal ? (rpmBar.topLabels ? parent.bottom : undefined): undefined
        // horizontal bar, labels on bottom --> start from parent.top
        anchors.top: horizontal ? (rpmBar.topLabels ? undefined : parent.top): undefined
        // horizontal bar --> start from parent.right | vertical bar, labels on bottom (left) --> start from parent.right
        anchors.right: horizontal ? parent.right : (rpmBar.topLabels ? undefined : parent.right)
        // vertical bar, labels on top (right) --> start from parent.left
        anchors.left: horizontal ? undefined : (rpmBar.topLabels ? parent.left : undefined)
        layer.enabled: true
    ...
    Rectangle {
        ...
        width: horizontal ? (rpmBar.value / rpmBar.maxValue) * parent.width : parent.width
        // horizontal bar -->  full parent.height | vertical bar --> parent.height * rpm/max rpm
        height: horizontal ? parent.height : (rpmBar.value / rpmBar.maxValue) * parent.height
        ...
        gradient: Gradient {
            GradientStop {
                position: 0.0
                color: rpmBar.value >= rpmBar.redlineValue ? "#993030" : "#663030"
            }
            GradientStop {
                position: 1.0
                color: "#480000"
    ...
    Rectangle {
        ...
        width: horizontal ? barEndThickness * parent.width : barEndLength * parent.width
        height: horizontal ? barEndLength * parent.height : barEndThickness * parent.height
        // anchor the 'across' center to the barFill's 'moving' end. Differs for horizontal and vertical bars.
        anchors.horizontalCenter: horizontal ? barFill.right : undefined
        anchors.verticalCenter: horizontal ? undefined : barFill.top
        // horizontal bar, labels on top --> start from parent.bottom
        anchors.bottom: horizontal ? (rpmBar.topLabels ? parent.bottom : undefined) : undefined
        // horizontal bar, labels on bottom --> start from parent.top
        anchors.top: horizontal ? (rpmBar.topLabels ? undefined : parent.top) : undefined
        ...
        anchors.right: horizontal ? undefined : (rpmBar.topLabels ? undefined : parent.right)
        layer.enabled: true
        layer.effect: MultiEffect {
            blurEnabled: true
            blurMax: 64
    ...
    Repeater {
        ...
        Rectangle {
            ...
            width: horizontal ? (index % rpmBar.bigTickEvery
                                 === 0 ? 2 : 1) : (index % rpmBar.bigTickEvery
                                                   === 0 ? 1.25 : 0.25) * parent.width
            height: horizontal ? (index % rpmBar.bigTickEvery === 0 ? 1.25 : 0.25)
                                 * parent.height : (index % rpmBar.bigTickEvery === 0 ? 2 : 1)
            color: "#e9e9e9"
            // horizontal bars --> arrange ticks on the x axis
            x: horizontal ? (index / (rpmBar.maxValue / rpmBar.tickStep)) * parent.width : 0
            // vertical bars --> arrange ticks on the y axis
            y: horizontal ? 0 : (1 - (index / (rpmBar.maxValue / rpmBar.tickStep))) * parent.height
            Text {
                text: index * rpmBar.tickStep
                visible: index % rpmBar.labelTickEvery === 0 ? true : false
                color: "#ffffff"
                font.pixelSize: Math.max(rpmBar.width, rpmBar.height) / 50
                font.family: tickFontName
                // horizontal bar, labels on top --> be above parent.top
                anchors.bottom: horizontal ? (rpmBar.topLabels ? parent.top : undefined) : undefined
                // horizontal bar, labels on bottom --> be below parent.bottom
                anchors.top: horizontal ? (rpmBar.topLabels ? undefined : parent.bottom) : undefined
                ...
                anchors.left: horizontal ? undefined : (rpmBar.topLabels ? parent.right : undefined)
                // vertical bar, labels on bottom (left) --> our right side should touch parent.left
                anchors.right: horizontal ? undefined : (rpmBar.topLabels ? undefined : parent.left)
            }
        }
    }
}
